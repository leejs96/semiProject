	...
	<c:forEach var="info" items="${info }">
		<form name="infoForm" method=post action=caregiver>
			<table id="infoTable" border=1>
				...
				<tr>
					<td>주소</td>
					<td><input type="text" name="address" value="${info.g_address }"></td>
				</tr>
					...
					<c:forEach var="repre" items="${representatives}" varStatus="r">
				       <tr id="repreTr">
							<c:if test="${r.index eq 0 }">
								<td class="repre" rowspan="${fn:length(representatives)}">대표서비스</td>
							</c:if>
							<td id="repreTd">
					...
					<c:forEach var="qual" items="${qualifications}" varStatus="status">
				       <tr id="qualTr">
				       		<c:if test="${status.index eq 0}">
								<td class="qual" rowspan="${fn:length(qualifications)}">자격증</td>
							</c:if>
							<td id='qualTd'>
				...
				<tr>
					<td>
						...
						<input type="submit" value="수정" id="validate">
					</td>
				</tr>
			</table>
		</form>
